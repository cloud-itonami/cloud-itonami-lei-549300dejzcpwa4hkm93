(ns tosmonitor.store
  ...
  This repo archives exactly ONE company (FirstGroup plc, LEI
  549300DEJZCPWA4HKM93) -- unlike `commitledger.store`'s multi-application
  ...
  The registered legal entity is FirstGroup plc, but the actual archived
  Terms of Use document -- and this actor's fixture data below -- lives on
  the First Bus operating-brand site (firstbus.co.uk), not the parent
  group's corporate site (firstgroupplc.com). This matches what the real
  archive (`80-data/public/tos.journal.edn`) already records; `demo-company`
  here deliberately uses `firstbus.co.uk` as its `:website` (not
  `firstgroupplc.com`) so it shares a base domain with `demo-baseline`'s
  real `:source-url` and this actor's `source-domain-mismatch-violations`
  check (see `tosmonitor.registry` ns docstring on its `.co.uk`/`.co.jp`
  two-part-TLD limitation) passes cleanly for this company's own demo/test
  fixture, matching every sibling actor's clean-lifecycle demo convention.
...
(def ^:private demo-full-text
  "Terms of Use\nWelcome to the First Bus website. By using this site to plan journeys, buy tickets, or check service updates, you agree to be bound by these Terms of Use, which First Bus (part of FirstGroup plc) may update from time to time. These Terms are governed by and construed in accordance with the laws of England and Wales.")

(defn demo-baseline []
  {:full-text    demo-full-text
   :source-url   "https://www.firstbus.co.uk/terms-use"
   :retrieved-at "2026-07-19T07:42:47Z"
